...
export function PaymentsView() {
  ...
  return (
    <div className="space-y-6 fade-up">
      ...
      <section className={`rounded-xl border p-5 ${form.credentialsConfigured && form.modeMatchesCredentials ? "border-emerald-200 bg-emerald-50/50" : "border-amber-200 bg-amber-50/60"}`}>
        <div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 h-5 w-5" /><div><p className="text-sm font-semibold">{form.credentialsConfigured && form.modeMatchesCredentials ? "Server credentials configured" : form.credentialsConfigured ? "Credential mode mismatch" : "Server credentials required"}</p><p className="mt-1 text-xs text-muted-foreground">{form.credentialsConfigured && form.modeMatchesCredentials ? `Razorpay key ${form.keyHint}. Secret values are never exposed.` : form.credentialsConfigured ? `The saved ${form.testMode ? "test" : "live"} mode does not match Razorpay key ${form.keyHint}. Checkout remains blocked until they match.` : "Add RAZORPAY_KEY_ID and RAZORPAY_KEY_SECRET to the backend environment before enabling checkout."}</p></div></div>
      </section>
      <section className="rounded-xl border border-border bg-card p-6 shadow-xs">
        ...
        <p className="mt-4 text-xs leading-relaxed text-muted-foreground">Authorized payments are captured by the backend when Automatic capture is enabled. Keep Razorpay Dashboard capture settings aligned as a fallback.</p>
      </section>
